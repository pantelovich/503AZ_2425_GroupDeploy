...
```javascript
db.urban_environment_data.insertOne({
  sensor_id: "AQ-001",
  location: "CivicNexus demo zone",
  air_quality_index: 42,
  recorded_at: new Date()
})

db.personnel_data.insertOne({
  staff_id: "ST-001",
  role: "Database administrator"
})

db.system_operational_logs.insertOne({
  event_type: "mongodb_replica_auth_test",
  status: "success",
  message: "Replica set and authenticated database access verified",
  recorded_at: new Date()
})
```
